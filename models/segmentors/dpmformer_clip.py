...
def make_contrastive_labels(batch_size):
    assert batch_size % 2 == 0, "Batch size must be an even number."
    
    # Create the positive mask
    half_bs = batch_size // 2
    positive_mask = torch.zeros(batch_size, batch_size).cuda()
    positive_mask[:half_bs, :half_bs] = 1  # Set the upper-left block to ones
    positive_mask.fill_diagonal_(0)         # Set the diagonal to zero
    
    # Create the negative mask
    negative_mask = torch.ones(batch_size, batch_size).cuda()
    negative_mask[:half_bs, :half_bs] = 0  # Upper-left block is zero for negatives
    negative_mask.fill_diagonal_(0)         # Set the diagonal to zero

    return positive_mask, negative_mask


@SEGMENTORS.register_module()
class dpmformer_CLIP(BaseSegmentor):
    ...
    def __init__(self,
        ...
        super(dpmformer_CLIP, self).__init__(init_cfg)
        ...
        text_length = self.text_encoder.context_length - self.context_length
        prompt_num = self.context_length
    
        self.texts = torch.cat([tokenize(c, context_length=text_length) for c in class_names]).to('cuda')
        self.contexts = nn.Parameter(torch.randn(1, prompt_num, token_embed_dim))
        self.gamma = nn.Parameter(torch.ones(text_dim) * 1e-4)

        self.meta_net = MLPMetaNet(token_embed_dim, token_embed_dim)
        ...
        self.metanet_contra_weight = metanet_loss_weights[0]
        self.metanet_contra_temp = metanet_loss_weights[1]        

    def extract_feat(self, img):
        x = self.backbone(img)
        return x

    def after_extract_feat(self, x, metanet_context=None):
        x_orig = list(x[:-1])
        ...
        if metanet_context is not None:
            text_embeddings = self.text_encoder(self.texts, context=self.contexts + metanet_context).expand(b_size, -1, -1)
        else:
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, **kwargs):
        img_org = kwargs['img_org']
        img = self.mixup_ratio * img + (1 - self.mixup_ratio) * img_org
        img = torch.cat([img_org, img], dim=0)
        img_metas = img_metas + img_metas
        gt_semantic_seg = gt_semantic_seg.repeat(2, 1, 1, 1)
        ...
        if self.meta_net is not None:
            metanet_context = self.meta_net(global_feat_0.squeeze(1))
        else:
            metanet_context = None
    
        x_orig, score_map, text_emb, global_feat = self.after_extract_feat(x, metanet_context.unsqueeze(1))
        ...
        if self.meta_net is not None:
            bs = metanet_context.size(0) // 2
            metanet_context = F.normalize(metanet_context, dim=-1)
            
            sim_matrix = torch.matmul(metanet_context, metanet_context.transpose(0, 1))

            positive_labels, negative_labels = make_contrastive_labels(metanet_context.size(0))
            # Calculate logits for positive pairs (numerator term in InfoNCE loss)
            pos_logits = sim_matrix * positive_labels / self.metanet_contra_temp
            pos_exp_sum = torch.exp(pos_logits[positive_labels.bool()]).sum(dim=-1)
            
            # Calculate logits for all pairs (denominator term in InfoNCE loss) 
            all_labels = (positive_labels + negative_labels)
            all_logits = sim_matrix * all_labels / self.metanet_contra_temp
            all_exp_sum = torch.exp(all_logits[all_labels.bool()]).sum(dim=-1)

            # Contrastive loss for each anchor
            loss_metanet_contrastive = -torch.log(pos_exp_sum / all_exp_sum).mean()
            loss_metanet_contrastive *= self.metanet_contra_weight
            loss_metanet_contrastive = {'loss' : loss_metanet_contrastive}
            losses.update(add_prefix(loss_metanet_contrastive, 'metanet.domain_contrastive'))

        # decode head loss
        gt_labels = kwargs['gt_labels'] + kwargs['gt_labels']
        gt_masks = kwargs['gt_masks'] + kwargs['gt_masks']
        loss_decode = self.decode_head.forward_train(
            x, text_emb, img_metas, gt_semantic_seg, self.train_cfg, gt_labels, gt_masks)
...
class MLPMetaNet(nn.Module):
    def __init__(self, in_channels=512, out_channels=512):
        super(MLPMetaNet, self).__init__()
        # 1D Conv to expand from 1 to 5 channels
