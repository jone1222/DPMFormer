...
class DPMFormerHead(BaseDecodeHead):
    ...
    def __init__(self,
                 ...
                 loss_mask=None,
                 loss_dice=None,
                 loss_cls_consistency=None,
                 loss_mask_consistency=None,
                 loss_pred_consistency=None,
                 train_cfg=None,
                 test_cfg=None,
                 init_cfg=None,
                 text_proj=None,
                 **kwargs):
        super(DPMFormerHead, self).__init__(
        ...
        self.loss_cls_consistency = build_loss(loss_cls_consistency)
        self.loss_mask_consistency = build_loss(loss_mask_consistency)
        self.loss_pred_consistency = build_loss(loss_pred_consistency)
    ...
    def loss_single(self, cls_scores, mask_preds, gt_labels_list,
        ...
        # consistency loss
        loss_cls_consistency = self.loss_cls_consistency(cls_scores)
        loss_mask_consistency = self.loss_mask_consistency(mask_preds)
        loss_pred_consistency = self.loss_pred_consistency(cls_scores, mask_preds)
        ...
        return loss_cls, loss_mask, loss_dice, loss_cls_consistency, loss_mask_consistency, loss_pred_consistency
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        losses_cls, losses_mask, losses_dice, losses_cls_consistency, losses_mask_consistency, losses_pred_consistency = multi_apply(
        ...
        loss_dict['loss_cls_consistency'] = losses_cls_consistency[-1]
        loss_dict['loss_mask_consistency'] = losses_mask_consistency[-1]
        loss_dict['loss_pred_consistency'] = losses_pred_consistency[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_mask_i, loss_dice_i, loss_cls_i_consistency, loss_mask_i_consistency, loss_pred_i_consistency in zip(
                losses_cls[:-1], losses_mask[:-1], losses_dice[:-1], losses_cls_consistency[:-1], losses_mask_consistency[:-1], losses_pred_consistency[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_mask'] = loss_mask_i
            loss_dict[f'd{num_dec_layer}.loss_dice'] = loss_dice_i
            loss_dict[f'd{num_dec_layer}.loss_cls_consistency'] = loss_cls_i_consistency
            loss_dict[f'd{num_dec_layer}.loss_mask_consistency'] = loss_mask_i_consistency
            loss_dict[f'd{num_dec_layer}.loss_pred_consistency'] = loss_pred_i_consistency
            num_dec_layer += 1
        return loss_dict
    ...
    def forward_head(self, decoder_out, mask_feature, attn_mask_target_size, get_similarity=False):
        ...
        if get_similarity is False:
            return cls_pred, mask_pred, attn_mask, sim
        else:
    ...
    def forward(self, feats, texts, img_metas, return_mask_features=False, get_similarity=False): ### need texts for pixel_decoder !
        ...
        cls_pred, mask_pred, attn_mask, sim = self.forward_head(
            query_feat, mask_features, multi_scale_memorys[0].shape[-2:], get_similarity=True)
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            cls_pred, mask_pred, attn_mask, sim = self.forward_head(
                query_feat, mask_features, multi_scale_memorys[
                    (i + 1) % self.num_transformer_feat_level].shape[-2:], get_similarity=True)
