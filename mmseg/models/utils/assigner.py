...
class MaskHungarianAssigner(BaseAssigner):
    ...
    def __init__(self,
                 ...
                 dice_cost=dict(type='DiceCost', weight=5.0),
                 mask_cost=dict(type='MaskFocalLossCost', weight=5.0)):
...
class IdentityAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        # extra_n = num_queries - self.num_cls if num_queries > self.num_cls else 0
        # if extra_n > 0 and gt_labels[-1] == (self.num_cls - 1):
        #     matched_row_inds = torch.concat((matched_row_inds, torch.arange(self.num_cls, num_queries, device='cuda')))   
        #     matched_col_inds = torch.concat((matched_col_inds, matched_col_inds[-1:].expand(extra_n)))
