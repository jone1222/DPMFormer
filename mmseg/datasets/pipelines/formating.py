        ...
        if 'gt_semantic_seg' in results:
            ...
            results['gt_semantic_seg'] = DC(
                to_tensor(results['gt_semantic_seg'][None,
                                                     ...].astype(np.int64)),
                stack=True)
        if 'img_org' in results:
            img_org = results['img_org']
            if len(img_org.shape) < 3:
                img_org = np.expand_dims(img_org, -1)
            img_org = np.ascontiguousarray(img_org.transpose(2, 0, 1))
            results['img_org'] = DC(to_tensor(img_org), stack=True)
        return results
